...
@section('content')
    <div class="card">
        ...
        <form action="{{ route('dashboard.inspection.update', $inspection->id) }}" method="post">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="form-group">
                    <label>Nama Jabatan</label>
                    <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $inspection->name) }}" required>
                </div>
                @error('name')
